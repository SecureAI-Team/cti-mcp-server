...
#### �️ 内置测试可视化界面 (MCP Inspector)
为了方便调试，生产集群已内置拉起一个 **MCP Inspector** 容器组件。
服务启动后，直接在浏览器访问：
`http://<公网IP>:5173`
该测试界面已自动配置好了内部 SSE 连接（`http://nginx:80/mcp`）并挂载了正确的 Auth Token，你可以直接在网页上即开即用地测试所有的 Tools、Resources 和 Prompts，无需手动配置。
